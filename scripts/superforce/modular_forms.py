...
def j_invariant(tau: complex, n_terms: int = 100) -> complex:
    ...
    # First few coefficients of j(τ) expansion (from OEIS A000521)
    coeffs = [
        -1,      # q^(-1)
        744,     # q^0
        196884,  # q^1
    ...
    # Compute sum: ∑ c_n q^(n-1) for n from 0 to n_terms
    result = 0
...
def verify_mckay_observation(n_coeffs: int = 10) -> Dict:
    ...
    McKay observation (1978):
      c(n) = ∑ dim(ρᵢ)  for specific subsets of representations
...
def thompson_series(g: str, tau: complex, n_terms: int = 10) -> complex:
    """
    Thompson series T_g(τ) for conjugacy class g of Monster

    The j-invariant is T_1(τ) (identity element series)
...
def main():
    ...
    print("  j(τ) = q^(-1) + 744 + 196884q + 21493760q² + ...")
    print()
