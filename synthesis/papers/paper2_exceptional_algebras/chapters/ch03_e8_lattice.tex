...
\begin{tikzpicture}[scale=1.2]
  ...
  % Gosset polytope 421: showing vertex-edge structure (3D projection schematic)

  % Central vertex
  \fill[blue!70!black] (0, 0) circle (3pt);
  \node at (0, -0.4) [font=\scriptsize] {vertex};

  % First layer: 56 neighbors (showing subset)
  \foreach \i in {0,30,60,...,330} {
    \pgfmathsetmacro{\radius}{1.2}
    \fill[red!60!black] ({\i}:\radius) circle (2pt);
    \draw[thick, blue!40] (0,0) -- ({\i}:\radius);
  }

  \node at (0, -2.2) [font=\scriptsize] {12 shown (of 56 nearest neighbors)};

  % Second layer: connections (showing partial structure)
  \foreach \i in {0,60,120,180,240,300} {
    \pgfmathsetmacro{\radius}{1.2}
    \pgfmathsetmacro{\nexti}{\i + 30}
    \draw[gray!50, thin] ({\i}:\radius) -- ({\nexti}:\radius);
  }

  % Outer annotation
  \draw[<->, thick, red] (0.3, 1.5) -- (1.5, 1.5);
  \node at (0.9, 1.8) [font=\scriptsize] {$d = \sqrt{2}$};

  % Right side: edge statistics
  \begin{scope}[xshift=5cm, yshift=0.5cm]
    \node at (0, 1.5) [font=\bfseries] {Gosset $4_{21}$ Structure};
    \node at (0, 1.0) [font=\small, align=left] {
      Vertices: 240 \\
      Edges: 6720 \\
      Coord. number: $k=56$
    };

    % Small polytope schematic
    \draw[thick, blue!60!black] (-0.8, -0.5) -- (0.8, -0.5) -- (0.4, -1.3) -- (-0.4, -1.3) -- cycle;
    \draw[thick, blue!60!black] (-0.8, -0.5) -- (-0.6, 0.2);
    \draw[thick, blue!60!black] (0.8, -0.5) -- (0.6, 0.2);
    \draw[thick, blue!60!black] (0.4, -1.3) -- (0.2, -1.8);
    \draw[thick, blue!60!black] (-0.4, -1.3) -- (-0.2, -1.8);
    \draw[thick, blue!60!black] (-0.6, 0.2) -- (0.6, 0.2) -- (0.2, -1.8) -- (-0.2, -1.8) -- cycle;

    \node at (0, -2.3) [font=\scriptsize] {8D polytope (schematic)};
  \end{scope}

\end{tikzpicture}
\caption{\textbf{Gosset $4_{21}$ polytope edge structure}. Left: Central vertex (blue) connects to 56 nearest neighbors (red, 12 shown) at distance $\sqrt{2}$. Each neighbor connects to others forming triangular 2-faces. Right: Summary statistics and schematic. Total: 240 vertices, 6720 edges, 60480 triangular faces.}
\label{fig:p2:gosset-polytope}
\end{figure}
...
\begin{tikzpicture}[scale=1.1]
  ...
  % Sphere packing visualization (2D slice of 8D packing)

  % Central sphere
  \fill[blue!30, opacity=0.7] (0, 0) circle (0.707);
  \draw[thick, blue!70!black] (0, 0) circle (0.707);
  \node at (0, 0) [font=\scriptsize] {$r$};

  % Surrounding spheres (kissing spheres)
  \foreach \i in {0,30,60,90,120,150,180,210,240,270,300,330} {
    \pgfmathsetmacro{\x}{1.414*cos(\i)}
    \pgfmathsetmacro{\y}{1.414*sin(\i)}
    \fill[red!30, opacity=0.6] (\x, \y) circle (0.707);
    \draw[thick, red!70!black] (\x, \y) circle (0.707);
  }

  % Distance annotation
  \draw[<->, thick, green!60!black] (0, 0) -- (1.414*cos(30), 1.414*sin(30));
  \node at (0.5, 0.5) [font=\scriptsize, fill=white] {$\sqrt{2}$};

  % Packing efficiency annotation
  \node at (0, -2.8) [font=\small] {2D slice of 8D optimal packing};
  \node at (0, -3.2) [font=\scriptsize] {Central sphere touches 12 neighbors (240 in 8D)};

  % Right side: density statistics
  \begin{scope}[xshift=6.5cm, yshift=0cm]
    \node at (0, 1.5) [font=\bfseries] {$E_8$ Packing Density};
    \node at (0, 0.8) [font=\small, align=left] {
      Dimension: 8 \\
      Kissing number: 240 \\
      Density: $\Delta_8 = \frac{\pi^4}{384}$
    };
    \node at (0, -0.3) [font=\small] {$\approx 25.37\%$};

    % Comparison bar
    \draw[thick] (-1.5, -1) rectangle (1.5, -0.7);
    \fill[green!60!black] (-1.5, -1) rectangle (0.31, -0.7);
    \node at (0, -1.4) [font=\scriptsize] {Optimal (Viazovska 2016)};

    % Dimensions where optimal is known
    \node at (0, -2.2) [font=\scriptsize, align=center] {
      Optimal known only in: \\
      1D, 2D, 3D, 8D, 24D
    };
  \end{scope}

\end{tikzpicture}
\caption{\textbf{$E_8$ lattice sphere packing} (2D slice). Central sphere (blue) at lattice point contacts 12 neighbors in this projection (240 in full 8D). Sphere radius $r = 1/\sqrt{2}$; minimal distance $\sqrt{2}$. Packing density $\Delta_8 = \pi^4/384 \approx 0.2537$ is optimal (Viazovska 2016, Fields Medal 2022).}
\label{fig:p2:e8-sphere-packing}
\end{figure}
...
\begin{tikzpicture}[scale=2.2]
  ...
  % Draw Coxeter plane projection of E8 roots (30-fold symmetry)
  % We'll show a simplified version with 30 roots arranged in 30-fold rotation

  % Outer ring: 30 roots with 30-fold symmetry
  \foreach \i in {1,...,30} {
    \pgfmathsetmacro{\angle}{360/30 * \i}
    \fill[blue!60!black] ({\angle}:1) circle (1.5pt);
  }

  % Inner ring: 30 roots
  \foreach \i in {1,...,30} {
    \pgfmathsetmacro{\angle}{360/30 * \i + 6}
    \fill[red!60!black] ({\angle}:0.618) circle (1.5pt);
  }

  % Innermost ring: 30 roots
  \foreach \i in {1,...,30} {
    \pgfmathsetmacro{\angle}{360/30 * \i}
    \fill[green!60!black] ({\angle}:0.382) circle (1.5pt);
  }

  % Central cluster (remaining 150 roots projected near center)
  \foreach \i in {1,...,40} {
    \pgfmathsetmacro{\angle}{rand*360}
    \pgfmathsetmacro{\radius}{0.15 + rand*0.12}
    \fill[orange!70!black, opacity=0.5] ({\angle}:\radius) circle (0.8pt);
  }

  % Symmetry lines (showing 30-fold rotation)
  \foreach \i in {0,30,60,90,120,150} {
    \draw[gray!40, dashed, very thin] (0,0) -- (\i:1.1);
  }

  % Annotations
  \node at (0, -1.4) [font=\small] {Coxeter plane projection};
  \node at (0, -1.7) [font=\scriptsize] {(240 roots $\to$ 2D with 30-fold symmetry)};

  % Scale indicator
  \draw[<->, thick] (1.25, -0.9) -- (1.25, 0.9);
  \node at (1.6, 0) [font=\scriptsize] {$h = 30$};

\end{tikzpicture}
\caption{\textbf{Coxeter plane projection of $E_8$ roots}. All 240 roots projected onto the Coxeter plane exhibit 30-fold rotational symmetry ($h = 30$). Colored rings show different projection radii; central cluster represents roots near projection center. Golden ratio $\phi = 0.618...$ appears in radial structure.}
\label{fig:p2:e8-coxeter-projection}
\end{figure}
