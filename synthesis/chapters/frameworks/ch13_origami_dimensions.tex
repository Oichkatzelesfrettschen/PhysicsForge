...
\input{modules/figures/fig_dimensional_folding}
...
\input{modules/figures/fig_fractal_lss}
...
\begin{example}[2D$\rightarrow$3D Origami Folding Calculation]
\label{ex:ch13:2d-3d-folding}

\textbf{Problem:} Calculate the 2D$\rightarrow$3D folding using origami function $f(x,y) = \sum_{n=1}^5 A_n \sin(k_n x) \sin(k_n y)$ with $k_n = 2\pi/(L_0 \varphi^n)$, amplitudes $A_n = A_0 / \varphi^{2n}$, box size $L_0 = 1$, $\varphi = (1+\sqrt{5})/2 = 1.618$ (golden ratio), and $A_0 = 0.1$. Evaluate $f(0.5, 0.5)$ and determine the fractal dimension via box-counting.
