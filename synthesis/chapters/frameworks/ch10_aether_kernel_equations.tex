...
\chapter{Scalar Field Kernel Equations}
...
\begin{align}
  K_{CD}^{(1)}(x) &= x \quad (\mathbb{R}) \label{eq:aether-kernel:cd-real} \eqtag{S}{MATH}{T} \\
  K_{CD}^{(2)}(x, y) &= x + iy \quad (\mathbb{C}) \label{eq:aether-kernel:cd-complex} \eqtag{S}{MATH}{T} \\
  K_{CD}^{(3)}(q) &= a + bi + cj + dk \quad (\mathbb{H}) \label{eq:aether-kernel:cd-quaternion} \eqtag{S}{MATH}{T} \\
  K_{CD}^{(4)}(o) &= \sum_{i=0}^{7} o_i e_i \quad (\mathbb{O}) \label{eq:aether-kernel:cd-octonion} \eqtag{S}{MATH}{T}
\end{align}
...
Components:
\begin{align}
  \dot{S}_{\text{therm}} &= \int d^3x \frac{\mathbf{J}_q \cdot \nabla T}{T^2} \label{eq:aether-kernel:thermal-entropy} \eqtag{S}{TD}{T} \\
  \dot{S}_{\text{quantum}} &= \frac{2\pi k_B}{\hbar} \sum_{i,j} \Gamma_{ij} |c_i|^2 |c_j|^2 \label{eq:aether-kernel:quantum-entropy} \eqtag{S}{TD}{T} \\
  \dot{S}_{\text{scalar}} &= \kappa_S \int d^3x \phi \nabla^2 \phi \label{eq:aether-kernel:scalar-entropy} \eqtag{S}{TD}{T}
\end{align}
