...
\begin{itemize}
  ...
  \item \textbf{Superconducting qubit arrays}: Eigenstate-ordered DTCs with programmable interactions; rapid tuning, large qubit counts (10--100 qubits)\cite{Mi2022TimeCrystal,Solfanelli2024DTC}.
  \item \textbf{NV centers in diamond}: Room-temperature operation, millisecond-scale coherence times, solid-state integration, and discrete time quasicrystal phases\cite{DTquasicrystal2025}.
\end{itemize}
...
\begin{enumerate}
  ...
  \item Modification of subharmonic period from pure integer multiples to fractional values in nodespace-coupled regimes (observed in various platforms)\cite{Solfanelli2024DTC,DTquasicrystal2025}.
...
\end{equation}
leading to \textit{anomalously long coherence times}. Note: The longest reported time crystal coherence ($>40$ minutes) was achieved in indium gallium arsenide nuclear spin systems\cite{Greilich2024TimeRobust}, not in NV centers, though NV-based time quasicrystals show promising stability\cite{DTquasicrystal2025}.
...
Compare $T_2^{\text{DTC}}$ (effective coherence time of DTC oscillations) to baseline $T_2$ (XY-8 or Ramsey). Prediction: $T_2^{\text{DTC}}/T_2 > 1$ if scalar-ZPE coupling stabilizes collective time-crystal state. Room-temperature operation and millisecond-scale intrinsic $T_2$ make NV centers ideal for detecting \textit{anomalous coherence extension} in time quasicrystal phases\cite{DTquasicrystal2025}.
...
\begin{itemize}
  \item Attach primary literature references (Zhang et al. 2017, Mi et al. 2022, Kyprianidis et al. 2021, Zu et al. 2025, Solfanelli et al. 2024, Greilich et al. 2024)\cite{Zhang2017TrappedIons,Mi2022TimeCrystal,Kyprianidis2021Prethermal,DTquasicrystal2025,Solfanelli2024DTC,Greilich2024TimeRobust}.
